...
We left off we were able to render the character's icon and move it around a
blank screen. Which is only impressive or exciting for about two minutes. Let's
add some non-player characters (NPCs) or as well call the Entities.
...
`defined or` assignment to set some defaults. This means that those parameters
aren't passed into the constructor we'll use these values instead.
...
was designed to default to encapsulation? This is an example of that.

If we don't explicitly make a way to access the variables they really shouldn't
be easily visible outside the scope of the class. In Perl's traditional object
...
not then you can skip ahead a little but otherwise let's take a second and
...
a contract with the code outside of the class that "if you call render(), you
don't need to worry about how render() is accomplished just know that it will
be". We'll see this play out as we move along and refactor and adapt our
classes as the game gets more complex.

With the traditional way of treating objects as just another data structure
sometimes people could put the object into a state that it wasn't expecting and
cause bugs. It encouraged tighter coupling of code outside the object and code
inside the object, and made it so you needed to be more careful with the
changes you made.
...
Entity object. We'll need to update our render method as well it should look
...
If we run this then,  everything should look exactly the same. That's the sign
of a good refactor, nothing changed.

We're still by ourselves though, let's add some more entities. Just after the
`$player` field declaration let's add a new one for `@npcs`
...
If we run things with this change, we're still gonna just see our player all
...
We're getting there. If we run things now we'll have our player and near by a
friend, Mr. N. They're still standing in a void though. We need to build a
...
We create a new function for creating floor tiles, we use `my` to make sure
that it's lexically scoped to inside the GameMap which is the only thing that
really cares about Tiles. We then use that to populate the tiles with a
multi-dimensional array of floor tiles.

We also will add a `render` method to the map to allow it to draw itself.
...
Now let's update the Engine to know about the GameMap. First we
add a field for it.
...
Now in theory if we've gotten everything right we and we run the app we should
...
a way to die yet, so I'm pretty sure we're not a ghost, we should fix that.
